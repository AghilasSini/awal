    ...
    def recognition(cls, sentence, language):
        ...
        for word in espeak.ipa_escape(espeak.get_raw_IPAPhoneme(sentence, language, ipa=1)).split(" "):
            for ph in word.split("_"):
                IPA_phonemes.append(espeak.convert_IPA_PhonSeq(
                    espeak.extract_ipa(Phonemes.IPA_Alphabet, ph)))

        array_sentence = utils.arr_cleaning(
        ...
        if not(len(IPA_phonemes) == len(array_sentence)):
            ...
            for w in array_sentence:
                phonemeSent = phonemeSent + " " + \
                    espeak.get_raw_IPAPhoneme(
                        w, language, ipa=1).replace(" ", "")
    ...
    def labels(cls, sentence, language):
        ...
        for phonemes in IPA_phonemes:
            ...
            for phoneme in phonemes.get_all_items():
                tPhoneme = SAMPAPhonemes.SAMPA_Alphabet.approximate_phoneme(
                    phoneme)
...
class LIAPHONPhonemes(Phonemes):
    Liaphon_Alphabet = roots.phonology_ipa_LiaphonAlphabet.get_instance()
    ...
    def labels(cls, sentence, language):
        ...
        # Conversion IPA vers LIAPHON
        for phonemes in IPA_phonemes:
            res = ""
            for phoneme in phonemes.get_all_items():
                tPhoneme = LIAPHONPhonemes.Liaphon_Alphabet.approximate_phoneme(
                    phoneme)
                tPhoneme = tPhoneme.replace("\\", "")
                tPhoneme = tPhoneme.replace("%", "")
                tPhoneme = tPhoneme.replace("\"", "")
            ...
            LIAPHON_phonemes.append(res)
            phonemes.destroy()

        return LIAPHON_phonemes





class CMUDICTPhonemes(Phonemes):
    Cmudict_Alphabet = roots.phonology_ipa_CmudictAlphabet.get_instance()
    ...
    def labels(cls, sentence, language):
        ...
        for phonemes in IPA_phonemes:
            res = ""
            for phoneme in phonemes.get_all_items():
                tPhoneme = CMUDICTPhonemes.Cmudict_Alphabet.approximate_phoneme(
                    phoneme)
                tPhoneme = tPhoneme.replace("\\", "")
                tPhoneme = tPhoneme.replace("%", "")
                tPhoneme = tPhoneme.replace("\"", "")
            ...
            CMUDICT_phonemes.append(res)
            phonemes.destroy()

        return CMUDICT_phonemes